...
function weeksAtNumberOne (arr) {
    ...
    for (let i = 0; numberOneSongs.length > i; i++) {
        if(!(`${numberOneSongs[i].song} - ${numberOneSongs[i].artist}` in frequencyCounter)) {
            frequencyCounter[`${numberOneSongs[i].song} - ${numberOneSongs[i].artist}`] = 0
        }
        frequencyCounter[`${numberOneSongs[i].song} - ${numberOneSongs[i].artist}`] += 1
        if (frequencyCounter[`${numberOneSongs[i].song} - ${numberOneSongs[i].artist}`] > maxCount) {
            maxValue = `${numberOneSongs[i].song} - ${numberOneSongs[i].artist}`;
            maxCount = frequencyCounter[`${numberOneSongs[i].song} - ${numberOneSongs[i].artist}`]
        }
    }
